...
Игра и панель администратора доступны на русском и английском. Кнопка `RU`/`EN`
в верхней части страницы переключает язык интерфейса, карты, уведомлений и
событий. Выбор сохраняется в браузере и действует на обеих страницах; при
первом открытии выбран английский. Названия объектов, заданные пользователями,
и технические JSON-ключи не переводятся.
...
## Новая модель юнитов (schema v4)

Боевой юнит состоит из индивидуальных экземпляров `PRODUCT` (формирований) и одного
прикреплённого артефакта-командира. Текущие/максимальные хиты и боевая мощь юнита
вычисляются на сервере из формирований, морали, тегов, артефактов и доктрин. Порог
боеспособности формирования — 30% его максимальных хитов. Обычный боевой урон
распределяется между живыми формированиями; артефакты не повреждаются и при гибели
юнита переходят в кораблекрушение вместе со знаниями.

Обнаружение выполняют только флоты: один бросок `1..currentHealth` за каждый
фактический переход в гекс или один раз за ход без движения. Запись об обнаружении
содержит гекс на момент проверки и не раскрывает новую позицию переместившейся цели.
Повышение максимальной десятины уменьшает мораль планеты на разницу ступеней;
аномалии с `moraleLoss` уменьшают мораль юнитов и планет в своём гексе в фазе UPDATE.

В админке есть редакторы тегов, их отношений, видов предметов, доктрин и состава
юнита. Новый готовый юнит создаётся через `POST /api/admin/units` с полями
`ownerPlayerId`, `domain`, `q`, `r`, `commanderKindId` и непустым
`formationKindIds[]`. Вид командира должен быть `ARTIFACT` с `commanderCapable`, а
виды формирований — боевыми `PRODUCT` с `baseCombatPower` и `maxHealth`.
Старые `POST /api/admin/fleets` и `POST /api/admin/armies`, создававшие юниты
напрямую из HP/БМ, возвращают `410`; существующие юниты продолжают загружаться.
`GET /api/admin/units/:id` возвращает производные показатели, а операции
`/api/admin/units/:id/{attach-formation,extract-formation,attach-artifact,detach-artifact,replace-commander,doctrines}`
меняют состав в фазе PLANNING. Определения редактируются через
`/api/admin/{tags,tag-relations,item-kinds,doctrines}`; отдельный экземпляр
формирования создаётся через `POST /api/admin/formations`.

Миграция `v3 → v4` добавляет новые коллекции, не пытаясь вывести характеристики
формирований из старых `UnitVariant` или стековых `REGIMENTS`. Старые юниты без
формирований сохраняют прежние боевые поля только как совместимость до явного
решения о конвертации. Назначать `UnitVariant` новым юнитам больше нельзя.
